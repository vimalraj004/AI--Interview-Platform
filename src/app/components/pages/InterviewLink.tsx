...
const InterviewLink = ({ interviewLinkId }: InterviewLinkProps) => {
  ...
  const sendThroughWhatsApp = () => {
    // window.location.href = `mailto:?subject=Invitation to AI Interview for ${interview.jobPosition}&body=Interview Link: ${url}`;
    const message = `Hey! I’ve scheduled an interview.Join here:${url}`;

    const whatsappUrl = `https://wa.me/?text=${encodeURIComponent(message)}`;

    window.open(whatsappUrl, "_blank");
  };
  const sendThroughEmail = () => {
    // Implementation for sending via email or other methods
    window.location.href = `mailto:?subject=Invitation to AI Interview for ${jobPosition}&body=Interview Link: ${url}`;
  };
  return (
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-5 sm:p-8 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <Input value={url} disabled className="truncate text-sm" />
            <Button
              onClick={onCopyLink}
